...
async function fetchIncidentList(): Promise<IncidentSummary[]> {
  const headers = await getAuthHeader();
  const res = await fetch('/api/incidents?limit=100&sort=priority', { headers });
  if (!res.ok) return [];
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--admin-text-primary)', marginBottom: 12 }}>
      {children}
    </h3>
  );
}
  ...
  return (
    <Card>
        ...
        {terms.map(t => (
          <div key={t.label}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 12, fontFamily: 'monospace', color: 'var(--admin-text-secondary)' }}>
      ...
      <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--admin-border)', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        ...
        <span style={{ fontSize: 24, fontWeight: 800, color: 'var(--admin-text-primary)' }}>
...
function StatusControls({
  ...
  const availableTransitions = useMemo(
    () => STATUS_TRANSITIONS[incident.status].filter(to => canTransition(incident.status, to)),
    [incident.status],
  );

  const selectedMergeCandidate = useMemo(
    () => mergeCandidates.find(c => c.incident_id === mergeTarget),
  ...
  const handleConfirmMerge = () => {
    ...
    const targetLabel = selectedMergeCandidate
      ...
      : mergeTarget;
    onTransition('DUPLICATE', `Incident merged as duplicate into incident ${mergeTarget} (${targetLabel})`);
    ...
    <Card>
        ...
        <div style={{ marginTop: 8 }}>
          {!mergeModalOpen ? (
            <button
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--admin-text-muted)'; e.currentTarget.style.color = 'var(--admin-text-primary)'; }}
            ...
            <div style={{ padding: 12, border: '1px solid var(--admin-border)', borderRadius: 8, background: 'var(--admin-bg-base)', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--admin-text-primary)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                <button
                  ...
                  style={{
                    ...
                    background: 'var(--bg-semantic-info)', border: '1px solid var(--color-semantic-info)',
                ...
                <button
                  type="button"
                  onClick={() => { setMergeModalOpen(false); setMergeTarget(''); }}
                  style={{ flex: 1, padding: '6px', borderRadius: 6, fontSize: 12, fontWeight: 600, background: 'var(--admin-bg-surface)', border: '1px solid var(--admin-border)', color: 'var(--admin-text-secondary)', cursor: 'pointer' }}
                >
                  Cancel
                </button>
              </div>
      ...
      <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--admin-border)', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--admin-text-primary)' }}>
    ...
    <Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {incident.status_history.map((entry, i) => (
          <div key={`${entry.at}-${i}`} style={{ display: 'flex', gap: 12, position: 'relative' }}>
            ...
            <div style={{ paddingBottom: 16, flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{
                  ...
                  fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em',
                  background: STATUS_COLORS[entry.to_status].bg,
                  color: STATUS_COLORS[entry.to_status].text,
                  border: `1px solid ${STATUS_COLORS[entry.to_status].border}`,
                }}>
                  {entry.to_status.replace('_', ' ')}
                </span>
                {entry.actor_name && (
                  <span style={{ fontSize: 10, color: 'var(--admin-text-muted)' }}>by {entry.actor_name}</span>
                )}
              </div>
              ...
              {entry.note && (
                <div style={{ fontSize: 12, color: 'var(--admin-text-secondary)', marginTop: 4 }}>{entry.note}</div>
              )}
            </div>
          </div>
        ))}
...
function SeverityConsensus({ consensus }: { consensus: IncidentDetail['severity_consensus'] }) {
  ...
  return (
    <Card>
      ...
      <div style={{ fontSize: 10, color: 'var(--admin-text-muted)', marginTop: -8, marginBottom: 12 }}>
...
function RecurrenceChain({ chain }: { chain: IncidentDetail['recurrence_chain'] }) {
  ...
  return (
    <div style={{ background: 'var(--bg-semantic-danger)', border: '1px solid var(--color-semantic-danger)', borderRadius: 12, padding: 20, boxShadow: 'var(--admin-shadow-card)' }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--admin-text-secondary)', marginBottom: 12 }}>
        ...
        {chain.length >= 2 ? 'Infrastructure replacement may be needed, not just patching.' : ''}
      </div>
...
function VerificationTally({ verification, status }: { verification: IncidentDetail['verification']; status: Status }) {
  if (!['RESOLVED','VERIFIED','REOPENED'].includes(status)) return null;
...
export default function IncidentDetailPage() {
  ...
  const {
    ...
    // Refresh every 30s so status changes by other staff are visible
    refetchInterval: 30_000,
    staleTime: 15_000,
  });

  /* Fetch the incident list for the merge-candidate dropdown */
  const { data: allIncidents = [] } = useQuery({
    queryKey: ['incidents-list-for-merge'],
    queryFn: fetchIncidentList,
    staleTime: 60_000,
  });

  /* PATCH mutation — handles status transitions and any other updates */
  const { mutate: applyPatch, isPending: isTransitioning } = useMutation({
    mutationFn: (patch: Record<string, unknown>) => patchIncident(incidentId, patch),
    onSuccess: (updated) => {
      // Update the detail cache directly with the fresh response
      queryClient.setQueryData(['incident', incidentId], updated);
      // Also invalidate the queue so the command centre shows the new status
  ...
  const handleTransition = (newStatus: Status, note = 'Status updated via admin dashboard') => {
    setTransitionError(null);
    applyPatch({ status: newStatus, note });
  };

  /* Merge candidates: same list minus this incident and terminal statuses */
    ...
    return (
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--admin-text-muted)' }}>Loading incident…</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <Link href="/admin" style={{ fontSize: 12, color: 'var(--color-semantic-info)', textDecoration: 'none' }}>
            ← Back to queue
          </Link>
        </div>
      </div>
    ...
    <div style={{ padding: '24px', maxWidth: 1280, margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <Link
            href="/admin"
            style={{ fontSize: 12, color: 'var(--color-semantic-info)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4, fontWeight: 500 }}
          >
          ...
          {incident.ward_name && (
            <div style={{ fontSize: 12, color: 'var(--admin-text-muted)' }}>Ward: {incident.ward_name}</div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          ...
          {/* Refresh button instead of the cosmetic "Live Sync" badge */}
          <button
            onClick={() => refetch()}
            title="Refresh incident"
            style={{
              display: 'flex', alignItems: 'center', gap: 6,
              background: 'var(--bg-semantic-success)', padding: '4px 10px', borderRadius: 20,
              border: '1px solid var(--color-semantic-success)', cursor: 'pointer',
            }}
          >
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--color-semantic-success)', boxShadow: '0 0 8px var(--color-semantic-success)' }} />
            <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--color-semantic-success)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Live
            </span>
          </button>
      ...
      {transitionError && (
        <div style={{
          ...
          background: 'var(--bg-semantic-danger)', color: 'var(--color-semantic-danger)',
          border: '1px solid var(--color-semantic-danger)',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        }}>
          <span>⚠ {transitionError}</span>
          <button onClick={() => setTransitionError(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-semantic-danger)', fontWeight: 700, fontSize: 16 }}>×</button>
        </div>
      )}
        ...
        <div style={{ gridColumn: 'span 12' }} className="lg:col-span-8 space-y-6">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <Card>
              <SectionTitle>
                Report Photos{' '}
                <span style={{ fontSize: 12, fontWeight: 400, color: 'var(--admin-text-muted)', marginLeft: 8 }}>
                  {incident.reports.length} of {incident.report_count} reports shown
                </span>
              </SectionTitle>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                {incident.reports.map(r => (
                  <div key={r.report_id} style={{ aspectRatio: '1', borderRadius: 8, overflow: 'hidden', position: 'relative', background: 'var(--admin-bg-active)', border: '1px solid var(--admin-border)' }}>
            ...
            <Card>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 }}>
                <span style={{ fontSize: 11, color: 'var(--admin-text-muted)' }}>
                  Centroid: {incident.centroid.lat.toFixed(4)}, {incident.centroid.lng.toFixed(4)}
                </span>
                <a
                  href={`https://www.google.com/maps?q=${incident.centroid.lat},${incident.centroid.lng}`}
                  target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 11, color: 'var(--color-semantic-info)', textDecoration: 'none' }}
                >
                  Open in Google Maps ↗
                </a>
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <Card>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12 }}>
                {[
                  { label: 'First reported', value: new Date(incident.first_reported_at).toLocaleDateString() },
                  { label: 'Report count', value: `${incident.report_count} unique reporters` },
                  { label: 'Age', value: `${incident.age_days.toFixed(1)} days` },
                  { label: 'Category', value: CATEGORY_LABEL[incident.category] },
                ].map(row => (
                  <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--admin-text-muted)' }}>{row.label}</span>
                    <span style={{ color: 'var(--admin-text-primary)', fontWeight: 500 }}>{row.value}</span>
                  </div>
                ))}
                {incident.flagged_mismatch && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--color-semantic-warning)', fontWeight: 600 }}>⚠ Flagged mismatch</span>
                    <span style={{ color: 'var(--color-semantic-warning)', fontSize: 10 }}>Category may be wrong</span>
                  </div>
                )}
              </div>
            </Card>
